...
using MF.Toolkit.Reloaded.Metaphor;
...
public class Mod : ModBase, IExports
{
    ...
    private readonly MetaphorLibrary _metaphorLib;
    ...
    private readonly MetaphorToolkit _metaphorToolkit;
    ...
    public Mod(ModContext context)
    {
        ...
        _metaphorLib = new MetaphorLibrary(scans!);
        _modLoader.AddOrReplaceController<IMetaphorLibrary>(owner, _metaphorLib);
        ...
        _metaphorToolkit = new MetaphorToolkit(modDir, _modders, _metaphorLib, scans!);
    ...
    private void OnModLoaded(IModV1 mod, IModConfigV1 config)
    {
        if (config.ModDependencies.Contains(modConfig.ModId))
        {
            ...
            {
                _metaphorToolkit.AddFolder(config.ModId, metaDir);
            }
        }
    }
    ...
    public Type[] GetTypes() => [ typeof(ISquirrel), typeof(IMetaphorLibrary), typeof(IMessage), typeof(IMetaphorToolkit) ];
    #endregion
